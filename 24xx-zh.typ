...
#let page-margin = (
  x: (148mm - 8.5pt * 42) / 3 - 8.5pt / 4,
  y: (210mm - line-height * 46) / 2,
)
#let column-gutter = page-margin.x + 8.5pt / 2
#set page(
  paper: "a5",
  margin: 0pt,
)
...
#block(
  ...
  = #text(fill: rgb(70%, 70%, 100%, 22.5%))[
    ...
    #h(-1.5pt)#text(18pt, top-edge: 22pt, "系统参考文档".codepoints().join(h(1fr)))#h(154.5pt)
  ]
]
...
*提升*：每次任务过后，每个角色可以提升一个技能（_无 ➡ d8 ➡ d10 ➡ d12_），并且获得 d6 信用点（₡）。
...
#block(
  ...
  #text(tracking: -1pt, "//") *开端*：解释设定的基本要素。如果其它地方没有解释清楚，那么给出一个让角色团结起来的理由，并提示他们应该把大部分时间用在什么活动上。
]
...
#grid(
  ...
  column-gutter: 0.5em,
  row-gutter: leading,
  [1–2], [什么也没找到。要想加入一个任务就得欠某个人点什么。],
  [3–4], [找到了一个任务，但有哪里不对。],
  [5-6], [有两个任务可供选择。]
)

#design-note[*寻找任务*：许多团队不需要为了获取报酬而找工作（比如军事单位）。不过，如果你的游戏确实采用了这种安排，那么危险的任务应该有更多回报，这样才付得起那些用来医疗，修理、更换损坏的装备，重骰坏任务，以及熬过找不到任务的空窗期的 1–3 信用点的“开支”。另外，上面这张表中的“欠下某个人”这个短语被有意模糊化了。或许值得将其澄清或引申到其它方面（比如把一个放贷人加进你的“联系人”表）。]
